...
description: Information on the settings and configuration options of your CS2 server from FSHOST
...
Counter-Strike servers can be configured widely. Basic settings can be configured via the **panel** site and via the settings page in the web interface. The advanced configuration is carried out via the gamemode config files such as gamemode_competitive.cfg, gamemode_custom.cfg, gamemode_casual.cfg, gamemode_cooperative.cfg and so on. 
